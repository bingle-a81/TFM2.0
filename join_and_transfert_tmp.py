...
def common_files_nomura(folder, machines):
    # TODO неполные программы
    folder_mashine_tmp = os.path.join(
        config.get_set_default("source"), config.get_set_default("DIR_TEMP"), folder
    )
                ...
                if os.path.exists(os.path.join(folder_mashine, file_name_2)):
                    with open(os.path.join(folder_mashine, file_name), "r") as f1:
                        ...
                        with open(os.path.join(folder_mashine, file_name_2), "r") as f2:
                            second_file = f2
                            with open(
                                os.path.join(folder_mashine_tmp, file_name), "w"
                            ) as f3:
                                common_file = f3
                                a = "\n$1\n"
...
def trans_other_machine(folder, machines):
    folder_mashine_tmp = os.path.join(
        config.get_set_default("source"), config.get_set_default("DIR_TEMP"), folder
    )
    simpe_functions.mk_folder(folder_mashine_tmp)
    for machine in machines:
        ...
        for file in simpe_functions.file_search(
            ...
            shutil.copy(
                file, os.path.join(os.path.join(folder_mashine_tmp))
            )  # клонируем папки в папку для разбора
